# ansible Playbook for adding the users to the machines
...
This playbook sets upp all the users on linux instead of the old
...
staff is everyone employed at linux, and contractors are people working for linux but not
under linux employment.
...
on all linux hosts. (see more under sudo privledges to give developers privledges on specific host.)
...
NEVER add a person at linux to users_to_keep or an account that is used to log in to
the linux servers.
...
playbooks/linux_users/roles/secure_sshd_access/templates/ilxsftp01.sshd_config.j2

## root password set
To set the root password on the linuxhosts you need some prerequisites.
...
```
ansible-vault edit ansible/playbooks/linux_users/roles/rootpwd/tasks/main.yml
```
...
Update the linux vault file with the password! Not the hashed one..
...
# current root password updated by linux_users playbook on all linux servers
